...
from trainer import generate_data,generate_test_data
import argparse
...
class TimeSeriesTransformer(nn.Module):
    ...
    def forward(self, src, tgt, src_mask=None, tgt_mask=None,train= True):
        ...
        if train == True:
            return self.forward_train(src, tgt, src_mask, tgt_mask)
        else: 
            return self.forward_test(src, src_mask)
        
    def forward_train(self, src, tgt, src_mask=None, tgt_mask=None):
    ...
    def forward_test(self, src, src_mask=None):
        """不需要tgt输入的推理函数"""
        # 源序列处理
        features = src[..., :-1]
        city_ids = src[..., -1].long()
        ...
        # 特征投影
        src_emb = self.input_proj(src_combined)
        ...
        dummy_tgt = torch.zeros(batch_size, seq_len, 1, device=src.device)
        tgt_emb = self.output_proj(dummy_tgt) + self.pos_enc
...
if __name__ == "__main__":
    ...
    config.dataset='SSP126_climate_data.xlsx'
    
    current_cities, X_test,  scaler, X = generate_test_data(config,scaler)
    
    output3=model(X_test.cuda(),None,train=False)
    
    output_data=np.abs(output3.cpu().detach().numpy())
    
    years = list(range(2028, 2099))  # 从2025年到2098年共71年
    cities = current_cities  # 使用已有的城市列表
    ...
    # 确保保存目录存在
    save_dir = config.save_dir if config.save_dir else "results"
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)

    # 构建文件名，包含目标变量信息
    target_name = "_".join(config.targets)
    output_filename = f"predictions_{target_name}_ssp126.xlsx"
    ...
    pivot_output_path =f"predictions_{target_name}_ssp126_pivot.xlsx"
    pivot_df.to_excel(pivot_output_path)
    print(f"透视表视图已保存至：{pivot_output_path}")
