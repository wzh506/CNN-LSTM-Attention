...
for province in provinces:
    ...
    Wg = 2.5 * features_data[:, 6] + np.random.normal(0, 5, 34)  # SPEI是第7个特征
...
# 模型训练
svr_models = [SVR(kernel='rbf', C=1.0, epsilon=0.1) for _ in targets]
for i, model in enumerate(svr_models):
    model.fit(X_train, y_train[:, i])

# 预测评估
y_pred = np.column_stack([model.predict(X_test) for model in svr_models])
