...
def train_model(config):
    ...
    save_dir = os.path.join("model", str(config.targets))
    # 训练模型
        ...
        for epoch in tqdm(range(config.epochs),desc='Training'):
            ...
            if (epoch + 1) % config.ckpt == 0:
                ...
                if config.test == True:
                    ...
                    model2.load_state_dict(torch.load(f'model/{config.targets}/DCLFormer.pth'))
                    # 测试模型
                    model2.to('cpu')
                    model2.eval()
                    ...
                    with torch.no_grad():
                        ...
                        y_test = y_test * existence_test.repeat(1, 1, y_test.shape[-1])  # 乘以存在性张量
                    ...
                    if config.targets == ['Wg','Wb']:
                        print("\n下面是Wb：")
                        for i, target in enumerate(y_test):
                            mse = mean_squared_error(y_test[i,0], preds[i,0])
                            rmse = np.sqrt(mse)
                            r2 = r2_score(y_test[i,0], preds[i,0]) #R2
                            print(f"first,MSE={mse:.2f}, RMSE={rmse:.2f}, R²={r2:.4f}")
                            results = calculate_metrics(preds[i,0], y_test[i,0])
                            print(results)

                        print("\n下面是Wg：")
                        for i, target in enumerate(y_test):
                            mse = mean_squared_error(y_test[i,1], preds[i,1])
                            rmse = np.sqrt(mse)
                            r2 = r2_score(y_test[i,1], preds[i,1]) #R2
                            print(f"first,MSE={mse:.2f}, RMSE={rmse:.2f}, R²={r2:.4f}")
                            results = calculate_metrics(preds[i,1], y_test[i,1])
                            print(results)
                    else:
                        print(f"\n下面是{config.targets}：")
                        for i, target in enumerate(y_test):
                            ...
                            results = calculate_metrics(preds[i,0], y_test[i,0])
                            print(results)
                torch.save(model.state_dict(),f'model/{config.targets}/DCLFormer_{epoch}.pth')
        if not os.path.exists(save_dir):
            os.makedirs(save_dir) 

        torch.save(model.state_dict(),f'model/{config.targets}/DCLFormer.pth')
