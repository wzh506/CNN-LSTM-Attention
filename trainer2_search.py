        ...
        for epoch in tqdm(range(config.epochs),desc='Training'):
            if config.mod == "DCLFormer":#正常训练方法
                ...
                if  test_flag==1: #初始化挺重要的
                    ...
                    r2 = r2_score(y_test3[-1,:], preds2[-1,:])
                    # 必须加限制了
            #数据增强,后半训练才可以使用
                else:
                    model.train()
            ...
            #数据增强
                    ...
                    # -1.0，-0.5，-0.5，-1.5
            #每一回合都要检查才可以
            if r2 > random.uniform(0.70,0.80):
                test_flag = 1 #别用第二个了
            else: 
                test_flag = 0 # 接着用第二个
...
################################################################################################################
        # 标记某几个城市
        # Jiaozuo, Langfang, Xinyang, Tai'an
        # 开始评测
        # 注意：cudnn 下 RNN/LSTM 在 eval() 模式做 backward 会报错：
        # RuntimeError: cudnn RNN backward can only be called in training mode
        # 因此归因阶段临时切到 train()，归因后再恢复原状态。

        was_training = model.training
        model.train()

        # 构造测试数据 [batch=23, window=4, cities=44, features=5]
        # 前20个是气象特征，最后一个是城市ID
        x = X_train
        ...
        target_city_idx = 0  #挨个分析所有城市即可拿下
        ...
        batch_size = x_input.size(0)    # 23
        window_size = x_input.size(1)   # 4
        num_cities = x_input.size(2)    # 44
        num_features = x_input.size(-1)    # 4

        if not was_training:
            model.eval()
        
        saliency = features.grad.abs() # 梯度绝对值代表特征重要性，维度: [23, 4, 44, 4]
        mean_saliency = saliency.mean(dim=0) # 对 batch 维度求平均，维度: [4, 44, 4]

        # 取出目标城市的特征重要性: [window_size=4, num_features=4]
        target_city_saliency = mean_saliency[:, target_city_idx, :] 

        print("=== 1. 哪些变量有显著滞后效应 (Which variables) ===")
        t_minus_3_importance = target_city_saliency[0, :] # 最远时刻 (t-3, 对应 window 索引 0) 
        t_minus_0_importance = target_city_saliency[-1, :] # 当前时刻 (t-0, 对应 window 索引 -1)

        # 防止你传入的特征数少于3，动态获取 Top-K
        k = min(3, num_features)
        topk_vars_t3 = torch.topk(t_minus_3_importance, k).indices
        topk_vars_t0 = torch.topk(t_minus_0_importance, k).indices

        print(f"对于城市{target_city_idx}:")
        print(f" -> 在 t-3 时刻(滞后最长)，起主导作用的特征索引是: {topk_vars_t3.tolist()}")
        print(f" -> 在 t-0 时刻(当期)，起主导作用的特征索引是: {topk_vars_t0.tolist()}")

        # ==========================================================
        # 算法 2：提取主要滞后长度 (Dominant lag lengths)
        # ==========================================================
        print("\n=== 2. 主要滞后长度 (Dominant lag lengths) ===")
        # 将 [1012, 4, 4] 还原为 [23, 44, 4, 4]
        temporal_attn_reshaped = temporal_attn.view(batch_size, num_cities, window_size, window_size)

        # 我们关心的是：在预测最后一步(索引 -1)时，模型把注意力分配给了哪些历史时刻
        # 对 batch 维度求平均 -> [44, 4]
        avg_temporal_lag_weights = temporal_attn_reshaped[:, :, -1, :].mean(dim=0) 

        lag_weights_city0 = avg_temporal_lag_weights[target_city_idx]
        print(f"城市{target_city_idx} 的时间注意力分配 (t-3, t-2, t-1, t-0):")
        print([round(val, 4) for val in lag_weights_city0.tolist()])

        # ==========================================================
        # 算法 3：提取空间变化 (How relationships vary spatially)
        # ==========================================================
        print("\n=== 3. 空间维度的注意力分配 (Spatial Variations) ===")
        # spatial_attn 已经是 [23, 44, 44] 了，直接跨 batch 求平均即可 -> [44, 44]
        avg_spatial_attn = spatial_attn.mean(dim=0) 

        # 查看哪些城市对 target_city 影响最大
        spatial_weights_city0 = avg_spatial_attn[target_city_idx]

        # 找出对目标城市影响最大的前 3 个城市
        top3_cities_idx = torch.topk(spatial_weights_city0, 3).indices
        print(f"对城市{target_city_idx} 估算影响最大的前3个城市索引是: {top3_cities_idx.tolist()}")



        # feature_names = ['Tmax', 'Tmin', 'Tave', 'RH']  #在前面定义了
        feature_names = config.features
        lag_names = ['t-3', 't-2', 't-1', 't-0']
        ...
        # ==========================================================
        # 1. 变量影响分布 (在 t-3 时刻的主导变量统计)
        # ==========================================================
        # mean_saliency 的维度是 [window_size=4, num_cities=44, num_features=4]
        # 我们提取最远滞后时刻 t-3 (索引为0) 的所有城市显著性
        for t in range(window_size):
            saliency = mean_saliency[t, :, :] # 维度: [44, 4],感觉也可以加权
            # saliency = mean_saliency.mean(dim=0) # 对窗口维度求平均，得到每个城市每个特征的平均重要性，维度: [44, 4]
            # 找出每个城市在 t-3 时刻最重要的特征索引 (Top-1)
            top1_feat_indices = torch.argmax(saliency, dim=1).cpu().numpy()

            # 统计每个特征成为 "Top-1" 的城市数量
            feat_counts = [np.sum(top1_feat_indices == i) for i in range(len(feature_names))]

            # --- 绘制变量影响直方图 ---
            fig, ax1 = plt.subplots(figsize=(8, 5))
            colors = sns.color_palette("Set2", len(feature_names))

            bars = ax1.bar(feature_names, feat_counts, color=colors, edgecolor='black', alpha=0.8)
            ax1.set_ylabel('Number of Cities', fontweight='bold')
            ax1.set_xlabel('Climate Variables', fontweight='bold')
            ax1.set_title(f'Lag Effect Across All Cities (t-{3-t} for {config.targets[0]})', pad=15, fontweight='bold')
            ax1.set_ylim(0, num_cities + 5)

            # 在柱子上方添加具体的数量标签
            for bar in bars:
                yval = bar.get_height()
                ax1.text(bar.get_x() + bar.get_width()/2, yval + 0.5, int(yval), ha='center', va='bottom', fontsize=11)

            plt.tight_layout()
            plt.savefig(f"results/{config.targets[0]}_global_variable_impact_sc{config.sc}_{t}.png", dpi=300, bbox_inches='tight')

        # ==========================================================
        # 2. 滞后长度总体分布 (箱线图 Boxplot)
        # ==========================================================
        # avg_temporal_lag_weights 的维度是 [44, 4]
        lag_weights_np = avg_temporal_lag_weights.detach().cpu().numpy()
        # 若希望 x 轴按 t-0 -> t-3 展示，需同时反转数据列与标签顺序
        lag_weights_np = lag_weights_np[:, ::-1]
        lag_names = lag_names[::-1] #只颠倒时间

        # 为了用 seaborn 画图，整理成一维数组形式
        df_lags = pd.DataFrame(lag_weights_np, columns=lag_names)

        # 将宽表转换为长表 (Melt) 以适配 sns.boxplot
        df_lags_melted = df_lags.melt(var_name='Lag Time', value_name='Attention Weight')

        # --- 绘制滞后长度分布箱线图 ---
        fig, ax2 = plt.subplots(figsize=(8, 5))

        # 使用箱线图展示分布，并叠加散点(swarmplot)展示每个城市的具体位置
        sns.boxplot(x='Lag Time', y='Attention Weight', data=df_lags_melted, 
                    width=0.5, palette="Pastel1", showfliers=False, ax=ax2)
        sns.stripplot(x='Lag Time', y='Attention Weight', data=df_lags_melted, 
                    size=4, color=".3", alpha=0.6, jitter=True, ax=ax2)

        # 高亮指定城市在每个滞后步长上的点
        highlight_cities = ["Jiaozuo", "Langfang", "Xinyang", "Tai'an"]
        city_to_idx = {str(city).strip().lower(): idx for idx, city in enumerate(current_cities)}
        highlight_specs = [
            ("焦作市", "#d62728", "o", "Jiaozuo"),
            ("廊坊市", "#1f77b4", "s", "Langfang"),
            ("信阳市", "#2ca02c", "D", "Xinyang"),
            ("泰安市", "#ff7f0e", "^", "Tai'an"),
        ]
        x_base = np.arange(len(lag_names))
        offset_unit = 0.06
        missing_highlight_cities = []

        for i, (city_name, color, marker, city_code) in enumerate(highlight_specs):
            city_idx = city_to_idx.get(city_name)
            if city_idx is None or city_idx >= lag_weights_np.shape[0]:
                missing_highlight_cities.append(city_name)
                continue

            # 给每个城市加轻微横向偏移，避免重叠
            x_offset = (i - (len(highlight_specs) - 1) / 2.0) * offset_unit
            x_vals = x_base + x_offset
            y_vals = lag_weights_np[city_idx, :]
            ...
            # 按你的要求：不在图中显示城市文字标注（避免方框乱码）

        if missing_highlight_cities:
            print(f"以下城市未在 current_cities 中找到，无法高亮: {missing_highlight_cities}")

        ax2.set_ylabel('Temporal Attention Weight', fontweight='bold')
        ax2.set_xlabel('Time Step', fontweight='bold')
        ax2.set_title(f'Temporal Lag Weights Across 44 Cities for {config.targets[0]}', pad=15, fontweight='bold')

        # 添加虚线辅助线，表示平均分配的基准线 (如果完全没有滞后侧重，权重应该是 1/4 = 0.25)
        ax2.axhline(1.0 / window_size, color='red', linestyle='--', alpha=0.5, label='Uniform Baseline (0.25)')
        ax2.legend(loc='lower left', fontsize=9)

        plt.tight_layout()
        plt.savefig(f"results/{config.targets[0]}_global_lag_distribution_sc{config.sc}_r1.png", dpi=300, bbox_inches='tight')
        ...
        plt.tight_layout()
        plt.savefig(f"results/{config.targets[0]}_global_lag_distribution_sc{config.sc}_r2.png", dpi=300, bbox_inches='tight')
