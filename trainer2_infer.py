...
def generate_train_data(config):
    # 读取数据
    df = pd.read_excel(config.dataset)
    window_size = config.window
    if config.sc is  None:
    ...
    return current_cities, X_train, y_train, scaler, X_tensor,y_tensor


def generate_test_data(config,scaler):
    # 读取数据
    df = pd.read_excel(config.infer_dataset)
    window_size = config.window
    if config.sc is  None:
        features = config.features
    else:
...
def train_model(config):
    ...
    current_cities, X_train, y_train, scaler, X_tensor,y_tensor = generate_train_data(config)
    # 读取数据
    current_cities2, X_infer, scaler, X_tensor_infer = generate_test_data(config,scaler)
...
##############################################################################################################################################
    ...
    if config.mod == 'DCLFormer':
        model = DCLFormer(input_size=X_train.shape[-1], output_size=y_train.shape[-1],window_size=config.window+1)
    ...
    if config.save_dir is None:
        ...
        if config.sc is not None:
            save_dir = os.path.join("model_infer",f"{config.mod}",str(config.targets)+f"_sc{config.sc}_{formatted_time}")
            txtname = f"{config.mod}"+str(config.targets)+f"_sc{config.sc}_{formatted_time}"
        else:
            save_dir = os.path.join("model_infer",f"{config.mod}", str(config.targets)+f"_{formatted_time}")
    ...
    if config.train == True:
        for epoch in tqdm(range(config.epochs),desc='Training'):
            if config.mod == "DCLFormer":#正常训练方法
                ...
                # if  test_flag==1: #初始化挺重要的 #这个时候没有训练集了
                model.train()
                optimizer.zero_grad()
    ...
    if config.test == True:#训练完毕，开始infer
        if config.mod == "DCLFormer":
            config.infer_dataset="SSP126_climate_data.xlsx"
            current_cities2, X_infer, scaler, X_tensor_infer = generate_test_data(config,scaler)
            #  current_cities2, X_infer, scaler, X_tensor_infer = generate_test_data(config,scaler)
            inputs_infer = X_infer.cuda()
            # existence_test = X_test[:,:,-1]  # 最后一个维度
            output3 = model(inputs_infer)
            output_data=np.abs(output3.cpu().detach().numpy())
            
            years = list(range(2028, 2099))  # 从2025年到2098年共71年
            cities = current_cities2  # 使用已有的城市列表
            ...
            for year_idx, year in enumerate(years):
                for city_idx, city in enumerate(cities):
                    # 获取该城市在该年份的预测结果
                    result_value = float(output_data[year_idx, city_idx, 0])
                    
                    # 添加到数据行列表
                    data_rows.append({
                        'city': city,
                        'year': year,
                        config.targets[0]: result_value
                    })
            ...
            # 确保保存目录存在
            save_dir = config.save_dir  if config.save_dir else save_dir
            if not os.path.exists(save_dir):
                os.makedirs(save_dir)

            # 构建文件名，包含目标变量信息
            target_name = "_".join(config.targets)
            base_name = config.infer_dataset.split(".")[0]
            output_filename = f"{save_dir}/predictions_{target_name}_{base_name}.xlsx"
            ...
            # 可选：创建透视表视图，以便于按城市和年份查看
            pivot_df = df.pivot(index='city', columns='year', values=config.targets[0])
            # pivot_output_path = os.path.join(save_dir, f"predictions_{target_name}_ssp126_pivot.xlsx")
            pivot_output_path =f"{save_dir}/predictions_{target_name}_{base_name}_pivot.xlsx"
            pivot_df.to_excel(pivot_output_path)
            print(f"透视表视图已保存至：{save_dir}/{pivot_output_path}")
            #SSP126_climate_data.xlsx
            ###################################################################################
            config.infer_dataset="SSP245_climate_data.xlsx"
            current_cities2, X_infer, scaler, X_tensor_infer = generate_test_data(config,scaler)
            #  current_cities2, X_infer, scaler, X_tensor_infer = generate_test_data(config,scaler)
            inputs_infer = X_infer.cuda()
            # existence_test = X_test[:,:,-1]  # 最后一个维度
            ...
            print(f"透视表视图已保存至：{save_dir}/{pivot_output_path}")
            #########################################################################################
            config.infer_dataset="SSP585_climate_data.xlsx"
            current_cities2, X_infer, scaler, X_tensor_infer = generate_test_data(config,scaler)
            #  current_cities2, X_infer, scaler, X_tensor_infer = generate_test_data(config,scaler)
            inputs_infer = X_infer.cuda()
            # existence_test = X_test[:,:,-1]  # 最后一个维度
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--epochs", default=30000, type=int, help="Total number of epochs." #用30000
    )
    parser.add_argument(
        "--lr",
        default=0.00001,
    ...
    parser.add_argument(
        "--ckpt", default=3000, type=int, help="Save model every ckpt epochs."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        help="Path to the infer dataset file.",
    )
    parser.add_argument(
        "--infer_dataset",
        default="SSP126_climate_data.xlsx",
        type=str,
