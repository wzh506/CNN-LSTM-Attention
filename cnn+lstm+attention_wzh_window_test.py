...
#     outputs = outputs * existence  # 乘以存在性张量
#     y_train = y_train * existence  # 乘以存在性张量
...
model2.load_state_dict(torch.load('model/DCLFormer_79999.pth'))
# 测试模型
model2.to('cpu')
model2.eval()
with torch.no_grad():
    ...
    outputs = preds * existence.repeat(1, 1, preds.shape[-1])   # 乘以存在性张量
    y_test = y_test * existence.repeat(1, 1, y_test.shape[-1])  # 乘以存在性张量
...
for i, target in enumerate(y_test):
    ...
    results = calculate_metrics(preds[i,:], y_test[i,:])
    print(results)
